...
export default (req, res, next) => { 
  ...
  let pass = true;
    ...
    const quantity = req.body.quatity;
  ...
  db.connect((err, client) => {
    const query = {
      text: 'select * from sales where name = $1',
      values: [req.params.name],
    };
    client.query(query, (err, result) => {
      if (err) res.status(400).json({ message: 'Sale Record already exists!'});
  ...
  if (pass === false) {
    res.status(400).json({
      Message : 'All sales properties must be inputed'
    });
